...
- **Code changes go through an orchestrated pipeline.** For non-trivial code changes, propose a demesne pipeline rather than editing directly: an Opus `sandbox_agent` orchestrator that runs explore → research → plan → orchestrator-determined numbered implementation phases → an in-sandbox validation gate → review/fix. The in-sandbox `make validate` (full lint+test+build, with linters pinned via `go tool` so it matches host and CI) is the **authoritative** deterministic gate — the host does not re-run it as a matter of course. The orchestrator commits the validated work on a branch in the shared `/workspace` plus a `/out/CHANGES.md`; the host **lands via branch fetch** (`git fetch <workspace>/repo <branch>` + ff-merge), runs one cheap in-repo `make validate` re-check, plus the host-only `make test-integration` (real podman/.env). See the `sandbox-feature-work` and `sandbox-quality-improvement` skills for the full pipeline.
