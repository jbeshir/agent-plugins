...
This survey targets the **source code** — logic, not prose quality. Its twin, `sandbox-prose-defect-survey`, runs the identical pipeline over the project's documentation, READMEs, comments, and generated text for *writing* flaws. Run them separately; a defect survey that tries to cover both code and prose in one taxonomy dilutes both. **One boundary case stays here, though: docs/code alignment.** A doc, README, or comment that *contradicts what the code actually does* (stale claims, lies, config defaults that have drifted) is a code-survey finding, because catching it takes reading the code to verify — keep a docs/code-accuracy lens in this survey's taxonomy. The prose survey owns how the text is *written*; this survey owns whether the text is *true of the code*.
...
3. **The taxonomy bar** — ~10 distinct types, grounded in **cited** commentary (not the model's priors), domain-appropriate; drop/merge inapplicable ones. One standing concern to keep represented even though the list is researched fresh: a **docs/code-alignment** lens — stale docs, READMEs, or comments that contradict what the code does. That belongs to this survey (the prose survey can't verify claims against the code).
