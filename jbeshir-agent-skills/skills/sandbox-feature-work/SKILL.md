...
6. **FINALIZE** — the orchestrator commits the validated work as **a single commit** on a branch in `/workspace/repo`, branched from the mounted HEAD (e.g. `pipeline/<short-task>`). Use multiple commits only when logical phases genuinely warrant separation; CHANGES.md must then list the commit range. Author the commit(s) with a placeholder identity (`pipeline@local` / `"Pipeline"`) — **do not** try to spoof the user's identity. The host re-authors after landing. It then **copies the committed repo into its own `/out`** so the host can reach it: `cp -a /workspace/repo /out/repo`, run **by the orchestrator itself** (plain `cp`, no toolchain needed) — *not* via a `sandbox_script` child, whose `/out` is `/out/child/<name>` and would strand the repo there. `/workspace` is torn down when the orchestrator exits; only `/out` (the returned `output_dir`) is persisted to the host, so the branch must live in a repo under `/out`. Finally it writes `/out/CHANGES.md` (what changed, how it was validated, the **branch name**, the **commit count** (and range if multiple), any caveats) and prints `DONE`. Committing on a branch is what makes host landing a fetch rather than a patch-apply.
...
9. **Output contract**: `/out/FINDINGS.md`, per-phase `/out/SUMMARY.md`, `/out/REVIEW.md` with a `PASS`/`CHANGES_NEEDED` verdict line, the validated work **committed as a single commit on a branch** (multiple commits only when logical phases warrant separation), authored as `Pipeline <pipeline@local>` (the host re-authors after landing — don't try to spoof the user's identity), **copied to `/out/repo`** by the orchestrator itself, and a final `/out/CHANGES.md` (naming the branch, the base commit it branched from, and the commit count) + `DONE`.
...
3. **Re-author the landed commits with the host's git identity.** The orchestrator commits as the placeholder `Pipeline <pipeline@local>`; rewrite the author of the just-landed range so the user's `user.name`/`user.email` is recorded.
   - Single commit (the default): `git -C <repo> commit --amend --reset-author --no-edit`.
   - Multiple commits: `git -C <repo> rebase <base> --exec "git commit --amend --reset-author --no-edit"`, where `<base>` is the commit the branch was branched from (CHANGES.md lists it).
   - Confirm with `git log -1 --format='%an <%ae>'` (or `git log <base>..HEAD --format='%an <%ae>'` for the multi-commit case).
4. **One cheap in-repo re-check.** Run `make validate` once in the real repo as a backstop, in-repo via the project's own target (not ad-hoc `go` commands). With tool versions pinned (below) this is a formality that passes first time; a failure means a version/environment gap to fix, not routine churn. **Read the log, not the exit code** — a trailing command can mask an earlier `make` failure with a 0 exit.
5. **Integration tests.** Run the project's host-only integration target (`make test-integration`) — the one check the sandbox can't do (it needs real podman + `.env`).
6. Manual quality pass (the `CLAUDE.md` checks); integrate/commit when the user asks.
