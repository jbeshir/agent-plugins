...
Capture the host commit identity to hand the orchestrator (`git -C … config user.name` / `user.email`), so the in-sandbox commit is authored correctly and the host needs no re-author. Confirm the stylesheet exists on the base ref: `git -C … ls-tree --name-only <base-ref> widgets/<slug>/src/styles.css`.
...
5. **Review-improve cycle** — a medium-tier `sandbox_agent` (`name=review01`) reads the finished tutorial against `styles.css` and judges: every excerpt is real and correctly anchored (no fabrication, no CSS feature credited that the widget doesn't use), the new-ground discipline holds (covered techniques are linked not re-taught), it explains *interaction* (cascade/specificity/nesting/inheritance) not just lists declarations, type-size/font-family/contrast choices are framed against readability floors (not presented as craft when they undercut legibility), prose is lean, and the ledger is precise. It writes `/out/REVIEW.md` ending in `PASS` or `CHANGES_NEEDED`. On `CHANGES_NEEDED`, re-run tie-together (or specific drafters) and re-review; cap at 3 rounds.
...
5. **States and accessibility** — focus rings, error/disabled treatment, contrast in both themes, `prefers-reduced-motion`; reference the shared patterns, teach only this widget's additions. Frame type size, font-family, and contrast against legibility floors (body ≥16px, sans for small UI text, WCAG AA + APCA Lc 75 in both themes). If the widget's own CSS sits below a floor, teach the floor and name the tradeoff honestly — don't present a sub-16px, low-contrast, or small-serif choice as an unqualified design win.
...
2. Create the branch ref locally **without touching the working tree**: `git -C /home/jbeshir/code/beshir-widgets fetch <output_dir>/repo "+tutorial/design-<slug>:refs/heads/tutorial/design-<slug>"` (fallback: `<output_dir>/child/<name>/repo`). The commit is already authored as the host identity, so there is no re-author step.
3. **Merged widget:** push the branch and open a PR — `git -C … push -u origin tutorial/design-<slug>`, then `mcp__github__create_pull_request` to `main` (body: which widget, the live URL, the CSS techniques it newly introduces). **Open widget PR:** re-confirm the PR is open and unmerged, then fast-forward the tutorial commit onto the widget's branch so it joins the existing PR — `git -C … push origin "tutorial/design-<slug>:<headRef>"` (no local switch).
